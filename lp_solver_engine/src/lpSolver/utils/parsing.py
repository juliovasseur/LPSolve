...
#    Updated: 2025/09/12 19:44:16 by Jvasseur         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
def _read_csv_dicts(path: str, required_headers: List[str]) -> List[Dict[str, str]]:
    """
    Lit un CSV en dictionnaire et vérifie les en-têtes.
    Ignore les lignes vides et les lignes commentaires (commençant par '#').
    Retourne une liste de lignes (dict) avec suppression des espaces superflus.
    """
    try:
        # 1) Nettoyage : on retire les lignes vides et celles qui commencent par '#'
        with open(path, "r", newline="", encoding="utf-8") as f:
            cleaned_lines: List[str] = []
            for raw in f:
                # Retire un éventuel BOM en début de fichier/ligne
                line = raw.lstrip("\ufeff")
                # Ignore lignes vides
                if not line.strip():
                    continue
                # Ignore lignes commentaires (même si précédées d'espaces)
                if line.lstrip().startswith("#"):
                    continue
                cleaned_lines.append(line)
        if not cleaned_lines:
            raise ParseError(f"{path}: fichier vide après suppression des commentaires/lignes vides.")

        # 2) On nourrit DictReader avec le contenu nettoyé
        reader = csv.DictReader(io.StringIO("".join(cleaned_lines)))
        if reader.fieldnames is None:
            raise ParseError(f"{path}: en-têtes manquantes.")
        headers = [h.strip() for h in reader.fieldnames]
        reader.fieldnames = headers  # trim headers
        ...
        for i, row in enumerate(reader, start=2):  # start=2 car ligne 1 = headers
            # Ignore lignes "vides" résiduelles (ex: ",,,")
            if row is None or all((v is None or (isinstance(v, str) and v.strip() == "")) for v in row.values()):
                continue
            # Trim cellule par cellule
            clean = {k: (v.strip() if isinstance(v, str) else v) for k, v in row.items()}
            # Par sécurité : si toute la ligne n'est qu'un commentaire (cas rare avec quotes), on saute
            first_non_empty = next((val for val in clean.values() if isinstance(val, str) and val != ""), "")
            if isinstance(first_non_empty, str) and first_non_empty.lstrip().startswith("#"):
                continue
            rows.append(clean)
        return rows
...
def parse_data_dir(data_dir: str) -> Dict[str, Any]:
    ...
    for i, row in enumerate(vars_rows, start=2):
        ...
        if low is None:
            low = 0.0  # défaut utile en LP: x >= 0
